...
export const TrackRecorderPanel: React.FC = () => {
  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-4 max-w-4xl mx-auto text-slate-100 pb-32 sm:pb-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
          <div className="bg-slate-950/60 border border-slate-800 p-3 rounded-xl">
            ...
            <div className="text-2xl font-black font-mono text-white mt-1">
              {activeTrack ? `${activeTrack.distanceKm.toFixed(2)}` : '0.00'} <span className="text-xs font-normal text-slate-400">km</span>
            </div>
          </div>
          ...
          <div className="bg-slate-950/60 border border-slate-800 p-3 rounded-xl">
            ...
            <div className="text-2xl font-black font-mono text-white mt-1">
              {isRecordingTrack ? (activeTrack?.points[activeTrack.points.length - 1]?.speed || 3.8) : '0.0'} <span className="text-xs font-normal text-slate-400">km/h</span>
            </div>
          </div>
          ...
          <div>
            {!isRecordingTrack ? (
              <div className="flex gap-2">
                <input
                  type="text"
                  placeholder="Nome da Trilha (ex: Patrulha Setor Norte - Canastra)"
                  value={trackNameInput}
                  onChange={(e) => setTrackNameInput(e.target.value)}
                  className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-sky-500"
                />
                <button
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
          <h3 className="font-bold text-base text-white">Trilhas Gravadas no Projeto ({savedTracks.length})</h3>
          <span className="text-xs text-slate-400">Total: {savedTracks.reduce((acc, t) => acc + t.distanceKm, 0).toFixed(2)} km</span>
        </div>
            ...
            >
              <div>
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-400 mt-1.5 font-mono">
                  <div>Dist: <b className="text-slate-200">{track.distanceKm.toFixed(2)} km</b></div>
                  <div>Vel. Média: <b className="text-slate-200">{track.avgSpeedKmh.toFixed(1)} km/h</b></div>
                  <div>Altimetria: <b className="text-emerald-400">+{track.elevationGainM}m</b> / <b className="text-rose-400">-{track.elevationLossM}m</b></div>
                  <div>Operador: <b className="text-slate-200">{track.userName}</b></div>
                </div>
              </div>

              <div className="flex items-center gap-2">
                <span className="text-[11px] text-slate-400 font-mono">
                  {new Date(track.startTime).toLocaleDateString('pt-BR')}
                </span>
              </div>
            </div>
          ))}
